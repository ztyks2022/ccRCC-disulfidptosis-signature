...
SUPP <- list(
  ...
  list(script="code/kirc/fig12_network.R", out="results/Fig9_network_v2", dst="Supplementary_Figure_S3"))
...
## S2 = somatic mutation oncoplot by risk group (pre-rendered by fig_tmb.R, png only)
if (file.exists("results/Fig_oncoplot.png")) {
  file.copy("results/Fig_oncoplot.png", sprintf("%s/Supplementary_Figure_S2.png", sdir), overwrite=TRUE)
  if (requireNamespace("magick", quietly=TRUE))
    magick::image_write(magick::image_convert(magick::image_read("results/Fig_oncoplot.png"), "tiff"),
                        path=sprintf("%s/Supplementary_Figure_S2.tiff", tifdir), format="tiff", density="300x300")
}
