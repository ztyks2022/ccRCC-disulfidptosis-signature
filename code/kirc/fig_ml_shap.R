...
if (!file.exists("results/kirc_ml_shap.rds")) {
...
pA <- ggplot(dfa, aes(alg, auc, fill=alg)) +
  ...
  scale_fill_manual(values=c(GBM=PAL$high, RF=PAL$n_mid, XGBoost=PAL$low), guide="none") +
...
## Panel D: SHAP beeswarm (manual — no overprinted mean values; those live in C)
shap_long <- SHAPforxgboost::shap.prep(xgb_model=res$xgb, X_train=res$Xm)
shap_long$variable <- factor(shap_long$variable, levels=names(sort(res$shap$mean_shap_score)))
pD <- ggplot(shap_long, aes(x=value, y=variable, colour=stdfvalue)) +
  geom_vline(xintercept=0, colour=PAL$n_mid, linewidth=.3) +
  geom_jitter(height=.20, size=.45, alpha=.55, stroke=0) +
  ...
  labs(x="SHAP value (impact on model output)", y=NULL,
       title="SHAP summary (directionality)") +
  theme_nat() + theme(legend.position="right", legend.title=element_text(size=6))
